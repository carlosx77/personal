...
where i.`IngredientName` like ('%Beef%') or i.`IngredientName`like ('%Garlic%')
...
-- Another simple way is to use conitions after the on clause
-- Be careful when using this way of filtering sets or tables
-- WARNNING!!!!!!!!!! this doest work correctly on MySQL!!!!!
...
select * from 
recipe_classes rc outer join recipes on rc.`RecipeClassID`=recipes.`RecipeClassID`

select *
from recipe_clases full outer join (
						select recipes.`RecipeTitle`, recipe_ingredients.`Amount`, measurements.`MeasurementDescription`, recipes.`RecipeClassID`, recipe_clases.`RecipeClassID`
						from recipes left join recipe_ingredients using (`RecipeID`) 
									 inner join measurements using (`MeasureAmountID`)
					) on (recipes.`RecipeClassID`= recipe_clases.`RecipeClassID`) 
				   full outer join (
...
having max(`ProductNumber`in(1, 2, 6, 11)) > 0 and max(`ProductNumber` in (10, 25, 26)) > 0 
